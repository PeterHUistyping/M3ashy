...
from .encoder.embedder import Embedder
from .mlp_models import MLP
from ..utils.device import device
from src.tools.merl_workflow.read_merl_list import read_text_from_file
...
class FrequencyEmbedder(nn.Module):
    ''' Sinusoidal positional encoding for scalars.
    '''
    ...
    def forward(self, x):
        ...
        x_unsqueezed = x.unsqueeze(-1).to(device, torch.float)  # (N, D, 1)
        scaled = (
...
class Transformer(nn.Module):
    ...
    def forward(self, x, t, y=None, train=False, x_prev=None):
        """
